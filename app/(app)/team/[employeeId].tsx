...
export default function EmployeeDetailScreen() {
  ...
  const query = useQuery({
    ...
    queryFn: () => mockSupabase.getEmployee(employeeId),
      ...
      {employee ? (
        <>
          <Card>
            ...
            <Text className="mt-2 text-sm text-slate-700">Assigned sites: {employee.assignedSites.join(', ')}</Text>
          </Card>
          <Card>
            <Text className="mb-1 text-base font-semibold text-slate-900">Training Transcript</Text>
            {employee.trainingTranscript.map((entry) => (
              <Text key={entry} className="text-sm text-slate-700">- {entry}</Text>
            ))}
          </Card>
          <Card>
            <Text className="text-base font-semibold text-slate-900">Inspection Activity</Text>
