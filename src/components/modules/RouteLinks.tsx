...
export const RouteLinks = ({ title, links }: { title: string; links: RouteLinkItem[] }) => (
  <Card>
    <Text className="mb-2 text-base font-semibold text-slate-900">{title}</Text>
        ...
        link.href ? (
          <Link href={link.href as any} key={link.href} asChild>
            <Pressable className="rounded-xl border border-brand-100 bg-brand-50 px-3 py-2">
              ...
              {link.description ? <Text className="text-xs text-slate-600">{link.description}</Text> : null}
            </Pressable>
          </Link>
        ) : (
          <Pressable
