...
export const RouteScreen = ({ title, description, links = [], bullets = [], stats = [] }: RouteScreenProps) => {
  return (
    <PageScaffold title={title} description={description}>
      {stats.length > 0 ? (
        <Card>
          <Text className="mb-2 text-base font-semibold text-slate-900">Snapshot</Text>
      ...
      {links.length > 0 ? (
        <Card>
          <Text className="mb-2 text-base font-semibold text-slate-900">Navigate</Text>
            ...
            {links.map((link) => (
              <Link key={link.href} href={link.href as any} asChild>
                <Pressable className="rounded-xl border border-brand-100 bg-brand-50 px-3 py-2">
                  <Text className="text-sm font-semibold text-brand-800">{link.label}</Text>
                </Pressable>
              </Link>
            ))}
          </View>
      ...
      {bullets.length > 0 ? (
        <Card>
          <Text className="mb-2 text-base font-semibold text-slate-900">Context</Text>
          {bullets.map((item) => (
